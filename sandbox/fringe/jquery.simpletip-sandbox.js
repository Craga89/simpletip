...
(function($)
{
   function Simpletip(root, contents, conf)
   {
      ...
      if(typeof conf.stem == 'string') conf.stem = { corner: conf.stem, size: 12, color: '#EBE4B4' }
      ...
      jQuery.extend(self,
      {
         ...
         createStem: function(corner)
         {
            ...
            else if(corner.search(/top|bottom/) !== -1)
            {
               if(corner.search(/Middle/) !== -1)
                  ...
                  self.stem.css({ marginLeft: Math.floor(self.tooltip.outerWidth() - conf.stem.size) });
            }
            ...
            if(!conf.hook.mouse)
            {
               ...
               if(typeof conf.hideOn == 'string')
               {
                  ...
                  if(conf.showOn !== hideOn)
                     self.root.unbind(hideOn).bind(hideOn, function(){ clearTimeout(showTimer); self.hide(); });
               ...
               else
               {
                  ...
                  self.tooltip.find('.'+elem).unbind(hideOn, self.hide).bind(hideOn, self.hide);
               }
            }
         ...
         assignCloseEvents: function(closeClass)
         {
            closeClass = closeClass || 'close';
            self.content.find('.'+closeClass).mousedown(function(){ self.hide(); return false; });
         },
