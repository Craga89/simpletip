...
}fnCallbackDraw(oSettings)});$(nNext).click(function(){if(oSettings.iDisplayStart+oSettings.iDisplayLength<oSettings.aiDisplay.length){oSettings.iDisplayStart+=oSettings.iDisplayLength
...
var aData=_fnGetDataMaster(oSettings);for(var j=0;j<iLoop;j++){oSettings.aoData[j].nTr.getElementsByTagName("td")[i].innerHTML=oSettings.aoColumns[i].fnRender({"iDataRow":j,"iDataColumn":i,"aData":aData})
...
var rpSearch=new RegExp(sInput,"i");for(i=oSettings.aiDisplay.length-1;i>=0;i--){if(!rpSearch.test(oSettings.aoData[oSettings.aiDisplay[i]]._aData[iColumn])){oSettings.aiDisplay.splice(i,1);
iIndexCorrector++}}}function _fnFilter(oSettings,sInput,iForce){var flag,i,j;if(typeof iForce=="undefined"||iForce==null){iForce=0
}if(typeof bRedraw=="undefined"||bRedraw==null){bRedraw=true}var asSearch=_fnEscapeRegex(sInput).split(" ");
...
oSettings.sPreviousSearch=sInput}else{if(oSettings.aiDisplay.length==oSettings.aiDisplayMaster.length||oSettings.sPreviousSearch.length>sInput.length||iForce==1){oSettings.aiDisplay.splice(0,oSettings.aiDisplay.length);
