/**
 * jquery.simpletip 2.0.0-beta2. A simple tooltip plugin
 * 
...
(function($)
{
   function Simpletip(root, contents, conf)
   {
      ...
      jQuery.extend(self,
      {
         ...
         assignEvents: function()
         {
            if(!conf.hook.mouse)
            {
               root.unbind(conf.showOn).bind(conf.showOn, function(event)
               { 
               ...
               if(typeof conf.hideOn == 'string')
               {
                  ...
                  if(conf.showOn !== hideOn)
                     root.unbind(hideOn).bind(hideOn, function(){ clearTimeout(showTimer); self.hide(); });
               }
            ...
            {
               root.mousemove(function(event){ self.updatePos(event); });
               root.bind(conf.showOn, function(event){ self.show(event); });
               root.mouseout(self.hide);
               
               self.assignCloseEvents();
            }
            
            //Assign focus events
            root.mouseover(self.focus);
         ...
         load: function(uri, data)
         {
            ...
            content.load(uri, data, function(){ self.assignCloseEvents(); conf.onContentLoad.call(self); });
         ...
         viewportAdjust: function(posX, posY, event)
         {
            ...
            var corner = conf.stem.corner || conf.hook.tooltip;
            ...
            if(overflow.leftMin || overflow.leftMax)
            {
               ...
               else if(overflow.leftMax)
               {
                  if(corner.search(/(top|bottom)Middle/) !== -1)
                     ...
                     posX = posX - conf.target.outerWidth() - tooltip.outerWidth() - (conf.offset[0] * 2);
               }
            ...
            if(overflow.topMin || overflow.topMax)
            {
               ...
               else if(overflow.topMax)
               {
                  if(corner.search(/(left|right)Middle/) !== -1)
                     posY = posY - (conf.target.outerHeight() / 2) - (tooltip.outerHeight() / 2) - (conf.offset[1] * 2);
                  else
                     posY = posY - conf.target.outerHeight() - tooltip.outerHeight() - (conf.offset[1] * 2);
               }   
   ...
   { 
      ...
      var defaultConf = {
         ...
         stem: { corner: false, color: '#EBE4B4', size: 12 },
         offset: [0, 0],
         viewport: false,
         hidden: true,
