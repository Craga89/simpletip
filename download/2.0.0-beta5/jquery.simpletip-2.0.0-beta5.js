/**
 * jquery.simpletip 2.0.0-beta5. A simple tooltip plugin
 * 
...
(function($) {
   function Simpletip(root, contents, conf)
   {
      ...
      $.extend(self,
      {
         ...
         createStem: function(corner)
         {
            ...
            $(self.tooltip).find('.'+conf.stemClass).remove();
            
            //Create stem element
            self.stem = $(document.createElement('div'))
   ...
   $.fn.simpletip = function(contents, options)
   { 
      ...
      //Setup configuration
      if(typeof options.stem == 'string') options.stem = { corner: options.stem };
      if(typeof options.border == 'string') options.border = { size: options.border };
      if(typeof options.title == 'string') options.title = { content: options.title };
      var config = $.extend(true, {}, $.fn.simpletip.defaults, options);
      
      //Create tooltips
      this.each(function()
      {
         var el = new Simpletip($(this), contents, config);
   ...
   $.fn.simpletip.defaults = {
      ...
      activeClass: 'active',
      width: false,
      border: { size: 3, radius: 0, color: '#ccc' },
